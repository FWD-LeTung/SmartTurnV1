...
def quantize_onnx_model(onnx_fp32_path, calibration_dataset, output_path):
    ...
    try:
        log.info("Quantizing ONNX model to INT16...")
...
def training_run(run_number):
    ...
    training_args = TrainingArguments(
        output_dir=f"./output3/{CONFIG['run_name']}",
    ...
    onnx_int16_path = os.path.join(export_path, "model_int16.onnx")
    ...
    if onnx_fp32_model_path is not None:
        ...
        quantized_onnx_path = quantize_onnx_model(onnx_fp32_model_path, calibration_dataset, onnx_int16_path)

        if quantized_onnx_path is not None:
            log.info("ONNX INT16 quantization successful")
            wandb.log({"export/onnx_int16_success": True})
            ...
            log.error("ONNX INT16 quantization failed")
            wandb.log({"export/onnx_int16_success": False})
    else:
