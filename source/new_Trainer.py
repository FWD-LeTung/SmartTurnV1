...
CONFIG = {
    ...
    "num_epochs": 10, # overfitting starts to occur beyond 4 epochs
    "train_batch_size": 64,
    "eval_batch_size": 32,
    "warmup_ratio": 0.2,
    "weight_decay": 0.01,
...
def training_run(run_number):
    ...
    CONFIG['run_name'] = f"v1-{now}_run{run_number}"
    
    log.info(f"Start Training process: {CONFIG['run_name']}")
    ...
    model = LumiSmartTurnV1Model.from_pretrained(CONFIG['model_name'], num_labels=1,ignore_mismatched_sizes=True)
    feature_extractor = WhisperFeatureExtractor(chunk_length=8)
    log_model_structure(model, CONFIG)

    datasets = prepare_datasets_ondemand(feature_extractor, CONFIG)
    training_args = TrainingArguments(
        output_dir=f"./output/{CONFIG['run_name']}",
    ...
    log.info("Done traing, done torch save final model(best model)")
    #----done with training, torch saved

    #----converting and quantization 
    ...
    log.infor("start converting to FP32 ONNX")
